  ...
  return (
    <div className="platform">
      <aside className="sidebar">
        ...
        <nav>
          {modules.map((item, index) => <button className={index === 0 ? 'nav-active' : ''} key={item}>{item}</button>)}
        </nav>
      ...
      <main className="workspace">
        ...
        <section className="two-column">
          <article className="panel dark-panel">
            ...
            <div className="action-row"><button>Send Invite</button><button className="outline">Copy LinkedIn DM</button></div>
          </article>
        ...
        <section className="panel">
          <div className="section-header">
            <div><span className="eyebrow">Power Partners Invite Center</span><h3>Priority Invite Queue</h3></div>
            <button>Add Contact</button>
          </div>
          <div className="table-wrap">
            <table>
              <thead><tr><th>Contact</th><th>Vertical</th><th>Market</th><th>Cluster</th><th>Status</th><th>Action</th></tr></thead>
              <tbody>
                {invites.map(([contact, vertical, market, cluster, status]) => (
                  <tr key={contact}><td>{contact}</td><td>{vertical}</td><td>{market}</td><td>{cluster}</td><td><span className="pill">{status}</span></td><td><button className="small">Send</button></td></tr>
                ))}
              </tbody>
            </table>
          </div>
        ...
        <section className="two-column lower">
          <article className="panel">
            ...
            <div className="form-grid"><input placeholder="Contact / Company" /><select><option>Vertical</option></select><select><option>Touchpoint Type</option></select><button>Log</button></div>
          </article>
          <article className="panel">
            <span className="eyebrow">Operations</span>
            <h3>Support Snapshot</h3>
            <div className="ops-list"><div><span>Open Ops Requests</span><strong>4</strong></div><div><span>Expenses Pending</span><strong>5</strong></div><div><span>Survey Follow-Ups</span><strong>3</strong></div></div>
          </article>
        </section>
      </main>
    </div>
